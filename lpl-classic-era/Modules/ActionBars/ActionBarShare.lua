    ...
    for index, segment in ipairs(segments) do
        if type(segment) == "table" then
            ...
            if importData then
                ...
                local existing = opts.existingActionBarID and LPL.ActionBarStore:Get(opts.existingActionBarID)
                if not existing and type(segmentName) == "string" and segmentName ~= "" then
                    existing = LPL.ActionBarStore:FindByName(segmentName)
                end
                if not existing and type(setName) == "string" and setName ~= "" then
                    existing = LPL.ActionBarStore:FindByName(setName)
                end
                if existing then
                    opts.existingActionBarID = existing.id
                end

                -- Keep the vault name on update; only use loadout path for brand-new sets.
