...
local function ApplySetData(setData, setName)
    ...
    if LPL.ActivateFeedback then
        LPL.ActivateFeedback:Play(string.format('Applied "%s" (some slots skipped)', setName))
    end
    print(string.format(
...
function LPL.ActionBarActivate:ApplySet(setID)
    ...
    local ok, err = ApplySetData(set, set.name)
    if ok and LPL.EditModePersist and LPL.EditModePersist.RememberActionBarSet then
        LPL.EditModePersist:RememberActionBarSet(setID)
    end
    return ok, err
end
