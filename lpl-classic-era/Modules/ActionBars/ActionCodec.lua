...
function LPL.ActionBarCodec:SanitizeDraft(draftSet)
    ...
    -- Drop Skyriding / possess slots so capture→apply never touches them on Era.
    local remove = {}
    for slot in pairs(draftSet.actions) do
        local slotID = tonumber(slot)
        if not slotID or not LPL.ActionBarDefinitions:IsManagedPlayerSlot(slotID) then
            remove[#remove + 1] = slot
        end
    end
    for i = 1, #remove do
        draftSet.actions[remove[i]] = nil
        draftSet.ignored[remove[i]] = nil
    end

    for slot = 121, 132 do
        draftSet.actions[slot] = nil
        draftSet.ignored[slot] = nil
    end
    for slot = 133, 144 do
        draftSet.actions[slot] = nil
...
function LPL.ActionBarCodec:BuildActionTableFromCursor()
    ...
    elseif cursorType == "spell" then
        -- GetCursorInfo("spell") is bookIndex, bookType, spellID, baseSpellID.
        -- Using bookIndex as a spell id stored Backstab for General-tab utilities.
        local rawId = a2
        if type(a4) == "number" and a4 > 0 then
            rawId = a4
        end
        local subType = a3 or "spell"
        if subType ~= "pet" then
            subType = "spell"
        end
        local dragName, dragIcon = GetSpellNameAndIcon(rawId)
        local id = NormalizeCapturedSpellID(rawId) or rawId
        if type(a5) == "number" and a5 > 0 then
            local baseNorm = NormalizeCapturedSpellID(a5)
