...
local function SyncEquipmentGlobal(equipment)
    ...
    LPLClassicEraDB.equipment = equipment
...
function LPL.EquipmentStore:NormalizeSetRecord(set)
    ...
    if LPL.SetRestrictions then
        ...
        if set.restrictions then
            set.restrictions.herotalents = nil
            set.restrictions.covenant = nil
            if not next(set.restrictions) then
                set.restrictions = {}
            end
        end
