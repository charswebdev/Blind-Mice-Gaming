    ...
    for index, segment in ipairs(segments) do
        if type(segment) == "table" then
            ...
            if importData then
                ...
                local existing = opts.existingEquipmentID and LPL.EquipmentStore:Get(opts.existingEquipmentID)
                if not existing and type(segmentName) == "string" and segmentName ~= "" then
                    existing = LPL.EquipmentStore:FindByName(segmentName)
                end
                if not existing and type(setName) == "string" and setName ~= "" then
                    existing = LPL.EquipmentStore:FindByName(setName)
                end
                if existing then
                    opts.existingEquipmentID = existing.id
                end
