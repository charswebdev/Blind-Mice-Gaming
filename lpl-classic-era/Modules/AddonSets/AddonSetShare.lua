    ...
    for index, segment in ipairs(segments) do
        if type(segment) == "table" then
            ...
            if importData then
                ...
                local existing = opts.existingAddonSetID and LPL.AddonSetStore:Get(opts.existingAddonSetID)
                if not existing and type(segmentName) == "string" and segmentName ~= "" then
                    existing = LPL.AddonSetStore:FindByName(segmentName)
                end
                if not existing and type(setName) == "string" and setName ~= "" then
                    existing = LPL.AddonSetStore:FindByName(setName)
                end
                if existing then
                    opts.existingAddonSetID = existing.id
                end
