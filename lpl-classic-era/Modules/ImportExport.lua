        ...
        LPL.ImportConfirmDialog:Show(preview, function(options)
            ...
            if preview.importKind == "loadout" then
                ...
                if options.talents or options.hero then
                    ...
                    if type(rawSource.dftalents) == "table" and #rawSource.dftalents > 1 and LPL.TalentShare then
                        for index = 2, #rawSource.dftalents do
                            ...
                            if type(segment) == "table" then
                                ...
                                local extra
                                if type(segment.tabs) == "table" and LPL.TalentShare.ApplyImportData then
                                    extra = LPL.TalentShare:ApplyImportData(segment, extraName, extraOpts)
                                elseif type(segment.string) == "string" and segment.string ~= "" then
