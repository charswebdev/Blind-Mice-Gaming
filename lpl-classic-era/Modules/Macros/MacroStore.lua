...
-- Classic Era Blizzard limits (Retail is 120/18).
LPL.MacroStore.ERA_ACCOUNT_MACROS = 18
LPL.MacroStore.ERA_CHARACTER_MACROS = 18
...
function LPL.MacroStore:GetMaxAccountMacros()
    return tonumber(MAX_ACCOUNT_MACROS) or self.ERA_ACCOUNT_MACROS
end

function LPL.MacroStore:GetMaxCharacterMacros()
    return tonumber(MAX_CHARACTER_MACROS) or self.ERA_CHARACTER_MACROS
end

function LPL.MacroStore:GetCharacterMacroStartIndex()
    return self:GetMaxAccountMacros() + 1
end
...
function LPL.MacroStore:FindLiveMacroIndexByName(wantName, scope)
    ...
    local charStart = self:GetCharacterMacroStartIndex()
...
function LPL.MacroStore:WriteToLiveMacro(draft)
    ...
    local maxAccount = self:GetMaxAccountMacros()
