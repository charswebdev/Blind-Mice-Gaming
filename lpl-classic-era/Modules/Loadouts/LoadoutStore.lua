...
function LPL.LoadoutStore:NormalizeSetRecord(set)
    ...
    if LPL.SetRestrictions then
        ...
        if set.restrictions and next(set.restrictions) then
            ...
            local build = buildID and LPL.TalentStore and LPL.TalentStore:Get(buildID)
...
function LPL.LoadoutStore:GetEffectiveSpecID(set)
    ...
    if type(restrictions) == "table" and next(restrictions) and type(restrictions.spec) == "table" then
        local inferred
...
function LPL.LoadoutStore:GetEffectiveHeroID(set)
    ...
    local restrictions = set.restrictions
    if type(restrictions) == "table" and next(restrictions) and type(restrictions.herotalents) == "table" then
        local inferred
