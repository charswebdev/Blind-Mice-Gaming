    ...
    local actionBar = LPL.TalentActionBar:Create(frame, {
        ...
        onActivate = function()
            if frame.viewMode == "tree" and frame.draftSet then
                if LPL.SetRestrictions and not LPL.SetRestrictions:AreValidForPlayerRecord(frame.draftSet) then
                    local summary = LPL.SetRestrictions:GetSummaryLine(frame.draftSet.restrictions)
                        or "another character, class, or race"
                    print("|cffff6060LPL:|r This action bar set is restricted to " .. summary .. ".")
                    return
                end
