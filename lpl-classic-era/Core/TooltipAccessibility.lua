...
function LPL:ShowGameTooltip(owner, spec)
    ...
    local function IsValidTooltipLink(link)
        if type(link) ~= "string" or link == "" then
            return false
        end
        -- Real hyperlinks look like "|Hspell:123|h[Name]|h" or "spell:123".
        -- Classic GetSpellLink can return a bare spell name — never pass that to SetHyperlink.
        if link:find("|H", 1, true) then
            return true
        end
        if link:match("^[%a%d]+:%d+") then
            return true
        end
        return false
    end
    ...
    if spec.hyperlink then
        ...
        if IsValidTooltipLink(link) then
