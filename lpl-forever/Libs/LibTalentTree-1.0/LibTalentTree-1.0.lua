...
-- GetNumClasses() is a count, not a max ID. Classic-style holes (no Death Knight = 6)
-- sit below later IDs such as Druid (11). Always scan through Evoker (13).
local function getMaxClassID()
    local maxClassID = GetNumClasses() or 0;
    if MAX_CLASSES and MAX_CLASSES > maxClassID then
        maxClassID = MAX_CLASSES;
    end
    if maxClassID < 13 then
        maxClassID = 13;
    end
    return maxClassID;
end
...
do
    local function initCache()
        ...
        for classID = 1, getMaxClassID() do
            local classFile = getClassFile(classID);
            if classFile then
    ...
    local function buildPartialCache(classID)
        ...
        for specIndex = 1, numSpecs do
            ...
            if not specID then
                specID = lastSpec and initialSpecs[classID] or nil;
            end
            if specID then
            ...
            nodes = nodes or C_Traits.GetTreeNodes(treeID) or {};
            local treeCurrencyInfo = C_Traits.GetTreeCurrencyInfo(configID, treeID, true) or {};
            local classCurrencyID = treeCurrencyInfo[1] and treeCurrencyInfo[1].traitCurrencyID;
            cache.treeCurrencyMap[treeID] = cache.treeCurrencyMap[treeID] or treeCurrencyInfo;
            -- Forever (and some classic-style trees) only expose a class currency.
            if cache.treeCurrencyMap[treeID][1] then
            ...
            for _, currencyInfo in ipairs(treeCurrencyInfo) do
                if currencyInfo.traitCurrencyID then
            ...
            for _, gateInfo in ipairs(treeInfo and treeInfo.gates or {}) do
            ...
            for _, nodeID in pairs(nodes) do
                ...
                if nodeInfo.isVisible then
                    ...
                    mergeTables(data.visibleEdges, nodeInfo.visibleEdges or {}, 'targetNode');

                    data.conditionIDs = data.conditionIDs or {};
                    mergeTables(data.conditionIDs, nodeInfo.conditionIDs or {});

                    data.groupIDs = data.groupIDs or {};
                    mergeTables(data.groupIDs, nodeInfo.groupIDs or {});
                    for entryIndex, entryID in pairs(nodeInfo.entryIDs or {}) do
                        ...
                        if not entryData[entryID] then
                            ...
                            if entryInfo.subTreeID then
                                ...
                                if subTreeInfo then
                                    ...
                                    local currencyInfo = subTreeInfo.traitCurrencyID and cache.treeCurrencyMap[treeID][subTreeInfo.traitCurrencyID];
        ...
        for _, nodeInfo in pairs(nodeData) do
            ...
            if nodeInfo.subTreeID then
                for specID, _ in pairs(nodeInfo.visibleForSpecs) do
                    nodeInfo.visibleForSpecs[specID] = nodeInfo.visibleForSpecs[specID] and cache.subTreeSpecMap[nodeInfo.subTreeID] and cache.subTreeSpecMap[nodeInfo.subTreeID][specID] or false;
                end
            end
        end
    end
    ...
    frame.numClasses = getMaxClassID();
