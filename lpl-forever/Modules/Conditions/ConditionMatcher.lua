...
local function IsDelveActive()
    if C_PartyInfo and C_PartyInfo.IsDelveInProgress then
        local active = SafeCall(C_PartyInfo.IsDelveInProgress)
        if active then
            return true
        end
    end
    if C_DelvesUI and C_DelvesUI.HasActiveDelve then
        local active = SafeCall(C_DelvesUI.HasActiveDelve)
        if active then
            return true
        end
    end
...
local function IsInsideHousing()
    if C_Housing and C_Housing.IsInsideHouse then
        if SafeCall(C_Housing.IsInsideHouse) then
            return true
        end
    end
    local inInstance, instanceType = IsInInstance()
    if inInstance and (instanceType == "interior" or instanceType == "neighborhood") then
        return true
    end
    return false
end
...
local function IsWarModeOn()
    if C_PvP and C_PvP.IsWarModeActive then
        return SafeCall(C_PvP.IsWarModeActive) and true or false
    end
    return false
end
...
local function LocationFlagsFromContext(ctx)
    ...
    if ctx.isDelve then
        flags.delve = true
    elseif ctx.instanceType == "party" then
        flags.dungeon = true
    elseif ctx.instanceType == "raid" then
        flags.raid = true
    elseif ctx.instanceType == "arena" then
        flags.arena = true
    elseif ctx.instanceType == "pvp" then
        flags.battleground = true
    elseif ctx.instanceType == "scenario" then
        flags.scenario = true
...
function LPL.ConditionMatcher:BuildContext()
    ...
    local isHousing = IsInsideHousing()
    local isRested = IsResting and IsResting() and true or false
    local warMode = IsWarModeOn()
...
local function LocationMatches(situations, ctx)
    ...
    for key, enabled in pairs(selected) do
        if enabled and current[key] then
            ...
            local needsDifficulty = (key == "dungeon" or key == "raid" or key == "arena"
                or key == "battleground" or key == "scenario" or key == "delve")
