...
function LPL.CooldownManagerEditor:Create(parent)
    ...
    title:SetText("Cooldown Manager Layout")
    ...
    hint:SetText("Paste a Blizzard Cooldown Manager string, or use Update to copy from your character.")
    ...
    input.onTextChanged = function(text, userInput)
        ...
        if userInput then
            frame.draftSet.layoutString = LPL.CooldownManagerCodec:NormalizeLayoutString(text)
            frame:RefreshStatus()
        end
    end
    ...
    function frame:RefreshStatus()
        ...
        layout = LPL.CooldownManagerCodec:NormalizeLayoutString(layout)
        if layout == "" then
            self.statusLabel:SetText("Empty layout — paste a string or Update from your character.")
        else
            self.statusLabel:SetText(string.format("Layout string ready (%d characters).", #layout))
        end
    end
    ...
    function frame:SyncDraftFromInput()
        ...
        self.draftSet.layoutString = LPL.CooldownManagerCodec:NormalizeLayoutString(self.input:GetText())
        LPL.CooldownManagerCodec:SanitizeDraft(self.draftSet)
        self:RefreshStatus()
    end
...
function LPL.CooldownManagerEditor:Destroy(editor)
    if not editor then
        return
    end
