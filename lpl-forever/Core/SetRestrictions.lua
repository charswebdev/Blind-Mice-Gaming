...
-- Full list-filter set used by vault tabs (mirrors LightPaws sidebar filters).
...
function LPL.SetRestrictions:AreValidForPlayer(restrictions)
    ...
    if restrictions.covenant and not BucketRestrictionMatches(restrictions.covenant, character:GetCovenantID()) then
        return false
    end
...
function LPL.SetRestrictions:GetSummaryLine(restrictions)
    ...
    if restrictions.covenant and type(restrictions.covenant) == "table" then
...
function LPL.SetRestrictions:GetFilterStorage(listKey)
    ...
    LPLForeverDB.listFilters = LPLForeverDB.listFilters or {}
    LPLForeverDB.listFilters[listKey] = LPLForeverDB.listFilters[listKey] or {}
    return LPLForeverDB.listFilters[listKey]
end
