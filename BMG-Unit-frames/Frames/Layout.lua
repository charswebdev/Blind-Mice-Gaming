...
function Frames.RelayoutParty()
    ...
    local cfg = Cfg("party")
    local w, h = cfg.w or 200, cfg.h or 52
...
function Frames.RelayoutRaid()
    ...
    local cfg = Cfg("raid")
    local w, h = cfg.w or 72, cfg.h or 28
...
function Frames.RelayoutArena()
    ...
    local cfg = Cfg("arena")
    local w, h = cfg.w or 160, cfg.h or 36
...
function Frames.Apply(profile)
    ...
    if Frames.headers.arena then
        ...
        Frames.RelayoutArena()
        if not enabled("arena") then
            Frames.headers.arena:Hide()
        end
    end
...
function Frames.Read(unit)
    ...
    elseif UnitIsGhost and UnitIsGhost(unit) then
        ...
        local cur, maxh, pct, unknown = UF.Factory.HealthInfo(unit)
        if unknown then
            parts[#parts + 1] = "health unknown"
        else
            parts[#parts + 1] = "health " .. tostring(math.floor(cur + 0.5)) .. " of " .. tostring(math.floor(maxh + 0.5)) .. ", " .. tostring(math.floor(pct * 100 + 0.5)) .. " percent"
        end
        if UF.Power and UF.Power.Primary then
            local primary = UF.Power.Primary(unit)
            if primary and primary.max and primary.max > 0 then
            ...
            if alt and alt.max and alt.max > 0 then
