...
function W.Stepper(parent, title, width)
    ...
    local box = CreateFrame("EditBox", nil, row, Template())
    box:SetSize(76, 28)
    box:SetPoint("RIGHT", plus, "LEFT", -8, 0)
    box:SetAutoFocus(false)
    box:SetMaxLetters(8)
    box:SetHistoryLines(1)
    box:SetAltArrowKeyMode(false)
    if box.SetTextInsets then
        box:SetTextInsets(4, 4, 0, 0)
    end
    local fontOk = box:SetFont(UF.Theme.FontPath(), 14, "OUTLINE")
    if not fontOk then
        box:SetFontObject(GameFontHighlight)
    end
    box:SetTextColor(UF.Theme.accent[1], UF.Theme.accent[2], UF.Theme.accent[3], 1)
    box:SetJustifyH("CENTER")
    W.Paint(box, UF.Theme.btn[1], UF.Theme.btn[2], UF.Theme.btn[3], 1, UF.Theme.accent[1], UF.Theme.accent[2], UF.Theme.accent[3])
    local minus = W.Button(row, "-", 32, 32)
    minus:SetPoint("RIGHT", box, "LEFT", -8, 0)
    row.minus = minus
    row.plus = plus
    row.Edit = box
    function row:SetValueText(text)
        row._shown = tostring(text or "")
        if box:HasFocus() then
            return
        end
        box:SetText(row._shown)
    end
    function row:SetOnCommit(fn)
        row._onCommit = fn
    end
    ...
    box:SetScript("OnEditFocusLost", function(self)
        W.Paint(self, UF.Theme.btn[1], UF.Theme.btn[2], UF.Theme.btn[3], 1, UF.Theme.accent[1], UF.Theme.accent[2], UF.Theme.accent[3])
        if row._skipCommit then
            row._skipCommit = nil
            return
        end
        local typed = self:GetText() or ""
        if typed == (row._shown or "") then
            return
        end
        if row._onCommit then
            row._onCommit(typed)
        end
    end)
    box:SetScript("OnEnter", function(self)
        GameTooltip:SetOwner(self, "ANCHOR_RIGHT")
        GameTooltip:SetText(title or "Number", UF.Theme.accent[1], UF.Theme.accent[2], UF.Theme.accent[3])
        GameTooltip:AddLine("Click the number and type a value. Press Enter to apply.", 0.9, 0.9, 0.9, true)
        GameTooltip:Show()
        if UF.Speech and UF.Speech.Say then
            UF.Speech.Say((title or "Number") .. ". Click the number to type a value.")
        end
    end)
