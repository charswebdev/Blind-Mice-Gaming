...
-- Retail Enum.PowerType uses CamelCase (HolyPower). Our tokens are HOLY_POWER.
local ENUM_TO_TOKEN = {
    Mana = "MANA",
    Rage = "RAGE",
    Focus = "FOCUS",
...
local function IdOf(token)
    if Enum and Enum.PowerType then
        if type(Enum.PowerType[token]) == "number" then
            return Enum.PowerType[token]
        end
        for camel, upper in pairs(ENUM_TO_TOKEN) do
            if upper == token and type(Enum.PowerType[camel]) == "number" then
                return Enum.PowerType[camel]
            end
        end
    end
...
local function TokenOf(id)
    ...
    if Enum and Enum.PowerType then
        for name, value in pairs(Enum.PowerType) do
            if value == id and type(name) == "string" then
                local token = ENUM_TO_TOKEN[name] or name
                if NAMES[token] then
                    TOKEN_BY_ID[id] = token
                    return token
                end
            end
        end
...
function Power.IsPipToken(token)
    return token and PIP_MAX[token] ~= nil
end
...
local function SafeNum(v)
    if UF.Compat and UF.Compat.SafeNumber then
        local n = UF.Compat.SafeNumber(v, 0)
        return n
    end
...
local function SafePower(unit, ptype)
    ...
    if UF.Compat and UF.Compat.IsSecretValue and UF.Compat.IsSecretValue(ptype) then
        return nil, nil
    end
...
local function RuneCount(unit)
    if unit ~= "player" then
        return HasMax(unit, "RUNES")
    end
    local max = 6
    local cur = 0
    if GetRuneCount then
        for i = 1, max do
            local ok, n = pcall(GetRuneCount, i)
            if ok then
                cur = cur + SafeNum(n)
            end
        end
        return true, cur, max, IdOf("RUNES")
    end
...
HasMax = function(unit, token)
    local id = IdOf(token)
    if not id then
        return false, 0, 0, nil
    end
    local cur, max = SafePower(unit, id)
    if UF.Compat and UF.Compat.IsUsablePositive and UF.Compat.IsUsablePositive(max) then
        return true, cur, max, id
    end
    if max ~= nil and (not UF.Compat or not UF.Compat.CanUseNumber or not UF.Compat.CanUseNumber(max)) then
        return true, cur, max, id
    end
    return false, cur or 0, max or 0, id
end
...
function Power.Primary(unit)
    ...
    local cur, max, pct
    if UnitPower then
        local ok, v = pcall(UnitPower, unit)
        if ok then
            cur = v
        end
    end
    if UnitPowerMax then
        local ok, v = pcall(UnitPowerMax, unit)
        if ok then
            max = v
        end
    end
    if UF.Compat and UF.Compat.PowerPercent then
        pct = UF.Compat.PowerPercent(unit, ptype)
    end
    local r, g, b
    if token then
    ...
    if PowerBarColor and ptype ~= nil then
        local secretType = UF.Compat and UF.Compat.IsSecretValue and UF.Compat.IsSecretValue(ptype)
        if not secretType then
...
function Power.Format(cur, max, mode, unknown, secret)
    mode = mode or "both"
    if mode == "none" then
        return ""
    end
