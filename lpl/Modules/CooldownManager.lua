...
local function CopyDraftFromSet(set)
    if not set then
        return LPL.CooldownManagerStore:CreateDraftSet(LPL.CooldownManagerStore:SuggestSetName())
    end
    return {
        name = set.name,
...
local function ApplyCooldownRestrictions(record)
...
local function DestroyEditor(frame)
    if frame.cdmEditor then
        LPL.CooldownManagerEditor:Destroy(frame.cdmEditor)
        frame.cdmEditor = nil
    end
end
...
local function BuildEditor(frame)
    ...
    frame.cdmEditor = LPL.CooldownManagerEditor:Create(frame.editorView)
    return frame.cdmEditor
end

local function RefreshEditor(frame)
    local editor = frame.cdmEditor
    if not editor then
...
local function SyncDraftFromEditor(frame)
    if frame.cdmEditor and frame.cdmEditor.SyncDraftFromInput then
        frame.cdmEditor:SyncDraftFromInput()
    end
    if frame.draftSet and LPL.CooldownManagerCodec then
        LPL.CooldownManagerCodec:SanitizeDraft(frame.draftSet)
    end
end
...
local function ShowListView(frame)
    ...
    local hasSets = #LPL.CooldownManagerStore:GetAll() > 0
...
local function ShowEditorView(frame, setID, isNew)
    ...
    if isNew then
        frame.activeSetID = nil
        frame.draftSet = LPL.CooldownManagerStore:CreateDraftSet(LPL.CooldownManagerStore:SuggestSetName())
    elseif setID then
    ...
    frame.actionBar:SetBuildName(frame.draftSet.name or LPL.CooldownManagerStore:SuggestSetName())
    frame.actionBar:UpdateTreeActions(frame.activeSetID ~= nil, true)
    RefreshEditor(frame)
end

function CooldownManagerModule.create(parent)
    local frame = CreateFrame("Frame", "LPLCooldownManagerModule", parent)
    ...
    local actionBar = LPL.TalentActionBar:Create(frame, {
        ...
        onActivate = function()
            if frame.viewMode == "editor" and frame.draftSet then
                ...
                if LPL.SetRestrictions and not LPL.SetRestrictions:AreValidForPlayerRecord(draftRecord) then
                    ...
                    print("|cffff6060LPL:|r This Cooldown Manager set is restricted to " .. summary .. ".")
                    return
                end
                LPL.CooldownManagerActivate:ApplyDraft(frame.draftSet, frame.actionBar:GetBuildName())
                if CooldownManagerModule.RefreshActiveSoon then
                    CooldownManagerModule.RefreshActiveSoon()
                end
            elseif frame.selectedSetID then
                local set = LPL.CooldownManagerStore:Get(frame.selectedSetID)
                ...
                LPL.CooldownManagerActivate:ApplySet(frame.selectedSetID)
                if CooldownManagerModule.RefreshActiveSoon then
                    CooldownManagerModule.RefreshActiveSoon()
                end
            else
                print("|cffff6060LPL:|r Select or edit a Cooldown Manager set to activate.")
            end
        ...
        onExport = function()
            ...
            if frame.viewMode == "editor" and frame.draftSet then
                ...
                exportText, err = LPL.CooldownManagerShare:ExportDraft(frame.draftSet, exportName)
            elseif frame.selectedSetID then
                local set = LPL.CooldownManagerStore:Get(frame.selectedSetID)
                if set then
                    exportName = set.name
                    exportText, err = LPL.CooldownManagerShare:ExportSet(set)
                end
            end
            ...
            if exportText and exportText ~= "" then
                ...
                print("|cffff6060LPL:|r " .. (err or "Select or edit a Cooldown Manager set to export."))
            end
        end,
        onImport = function()
        ...
        onUpdate = function()
            ...
            local ok, err = LPL.CooldownManagerCodec:CanCaptureFromCharacter()
            if not ok then
                print("|cffff6060LPL:|r " .. (err or "Cannot update from character."))
                return
            end
            local captured, captureErr = LPL.CooldownManagerCodec:CaptureFromCharacter(frame.draftSet)
            if not captured then
                print("|cffff6060LPL:|r " .. (captureErr or "Failed to read Cooldown Manager layout."))
                return
            end
            RefreshEditor(frame)
            print("|cff33cc33LPL:|r Loaded Cooldown Manager layout from your character.")
        end,
        onReset = function()
            if not frame.draftSet then
                return
        ...
        onSave = function()
            ...
            LPL.CooldownManagerStore:SaveFromEditor(frame.activeSetID, name, frame.draftSet, function(setID)
                frame.selectedSetID = setID
                ShowListView(frame)
            end)
        end,
        ...
        onDelete = function()
            ...
            LPL.CooldownManagerStore:ConfirmDelete(setID, function()
    ...
    local setList = LPL.CooldownManagerSetList:Create(listView, 0)
    ...
    LPL.RestrictionsMenu:Attach(actionBar.limitsButton, function()
        ...
        SyncDraftFromEditor(frame)
        ApplyCooldownRestrictions(frame.draftSet)
        if frame.activeSetID and frame.draftSet then
            local set = LPL.CooldownManagerStore:Get(frame.activeSetID)
            if set then
                set.restrictions = LPL.SetRestrictions:CopyRestrictions(frame.draftSet.restrictions)
                ApplyCooldownRestrictions(set)
                LPL.CooldownManagerStore:CommitSet(set)
            end
        end
        RefreshEditor(frame)
    ...
    LPL.RestrictionsMenu:Attach(actionBar.listLimitsButton, function()
        return frame.selectedSetID and LPL.CooldownManagerStore:Get(frame.selectedSetID)
    end, function(record)
    ...
    frame:Hide()
    CooldownManagerModule.instance = frame

    if not CooldownManagerModule.eventFrame then
        ...
        eventFrame:SetScript("OnEvent", function()
            local instance = CooldownManagerModule.instance
            if instance and instance:IsShown() and instance.viewMode == "list" and instance.setList then
                instance.setList:Refresh()
            end
        end)
        -- Refresh green dots shortly after activate (layout apply can lag a tick).
        CooldownManagerModule.RefreshActiveSoon = function()
        ...
        CooldownManagerModule.eventFrame = eventFrame
    end

    return frame
end

function CooldownManagerModule:OnShow()
...
LPL.Modules:Register({
    id = CooldownManagerModule.id,
    label = CooldownManagerModule.label,
    description = CooldownManagerModule.description,
    icon = CooldownManagerModule.icon,
    iconStem = CooldownManagerModule.iconStem,
    order = CooldownManagerModule.order,
    create = CooldownManagerModule.create,
    OnShow = function()
        CooldownManagerModule:OnShow()
    end,
})
