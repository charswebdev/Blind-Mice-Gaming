...
local function ApplyLoadoutRestrictions(record)
    ...
    if record.restrictions and next(record.restrictions) then
        ...
        local build = buildIDs[1] and LPL.BuildStore and LPL.BuildStore:Get(buildIDs[1])
        if build and build.classID then
            record.classID = tonumber(build.classID)
            record.specID = tonumber(build.specID)
            record.subTreeID = tonumber(build.subTreeID)
        else
            record.classID = nil
            record.specID = nil
