...
function LPL.EquipmentItemInfo:GetUpgradeInfo(itemLink)
    ...
    return GetUpgradeInfoFromTooltipAPI(itemLink)
        or GetUpgradeInfoFromScanTooltip(itemLink)
end

function LPL.EquipmentItemInfo:GetUpgradeText(itemLink)
    local info = self:GetUpgradeInfo(itemLink)
    if not info or not info.trackString or not info.currentLevel or not info.maxLevel then
        return nil
    end

    return string.format("[%s %d/%d]", info.trackString, info.currentLevel, info.maxLevel)
end
