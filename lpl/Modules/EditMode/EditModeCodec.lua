...
local function GetPresetLayoutCount()
    ...
    if Enum and Enum.EditModePresetLayouts and Enum.EditModePresetLayouts.Classic then
        return (Enum.EditModePresetLayouts.Classic or 1) + 1
    end
    return 2
end
...
local function EnsureEditModeReady()
    ...
    if not EditModeManagerFrame then
        return nil, "Edit Mode UI is unavailable."
    end
...
function LPL.EditModeCodec:ApplyLayoutString(layoutString, options)
    ...
    local activeIndex = GetPresetLayoutCount() + #editModeLayouts.layouts
