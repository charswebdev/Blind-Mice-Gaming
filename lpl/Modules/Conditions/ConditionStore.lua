...
function LPL.ConditionStore:GetStoreForLinkType(linkType)
    ...
    elseif linkType == "talent" then
        return LPL.BuildStore
    elseif linkType == "pvpTalent" then
        return LPL.PvpTalentStore
    elseif linkType == "actionBar" then
        return LPL.ActionBarStore
    elseif linkType == "equipment" then
        return LPL.EquipmentStore
    elseif linkType == "cooldownManager" then
        return LPL.CooldownManagerStore
...
function LPL.ConditionStore:GetRecordSpecID(record)
    ...
    -- Loadouts: use first linked talent build's spec when unique.
    if type(record.talentBuildIDs) == "table" and LPL.BuildStore then
        local only
    ...
    if record.talentBuildID and LPL.BuildStore then
        local build = LPL.BuildStore:Get(record.talentBuildID)
