...
AllQuestForeverCharDB = AllQuestForeverCharDB
local function Rec()
    local c = AQ.DB.Char()
...
function Debug.Record()
    ...
    log[#log + 1] = entry
    AQ:Print("Recorded " .. tostring(#entry.quests) .. " log quests at map " .. tostring(mapID) .. ". Saved in character DB (AllQuestForeverCharDB.debugLog).")
...
function Debug.DumpQuestLines()
    ...
    AQ:Print("Dumped " .. tostring(#lines) .. " C_QuestLine entries for map " .. tostring(mapID) .. ". Copy AllQuestForeverCharDB.debugLog.extractor into tools JSON.")
