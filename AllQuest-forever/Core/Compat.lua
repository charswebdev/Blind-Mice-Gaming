...
function Compat.GetFlavor()
    if AQ.Flavor and AQ.Flavor.id then
        return AQ.Flavor.id
    end
    return "forever"
end
...
function Compat.IsRetail()
    -- Forever is Mainline (Camelot). Tracker APIs match Retail / Midnight.
    local flavor = Compat.GetFlavor()
    return flavor == "retail" or flavor == "forever"
end
...
function Compat.AllowedExpansionIDs()
    ...
    if flavor == "forever" then
        -- Forever quest IDs are not Midnight (11) or Classic Era (0).
        return { 60 }
    end
