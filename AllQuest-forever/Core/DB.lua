...
local function EnsureRoot()
    if type(AllQuestForeverDB) ~= "table" then
        AllQuestForeverDB = {}
    end
    if type(AllQuestForeverDB.profiles) ~= "table" then
        local settings = {}
        local remove = {}
        for k, v in pairs(AllQuestForeverDB) do
        ...
        for i = 1, #remove do
            AllQuestForeverDB[remove[i]] = nil
        end
        AllQuestForeverDB.profiles = {
            Default = CopyDefaults(defaults, settings),
        }
    end
    local any = false
    for name, prof in pairs(AllQuestForeverDB.profiles) do
    ...
    if not any then
        AllQuestForeverDB.profiles.Default = CopyDefaults(defaults, {})
    end
    return AllQuestForeverDB
end

local function EnsureChar()
    if type(AllQuestForeverCharDB) ~= "table" then
        AllQuestForeverCharDB = {}
    end
    CopyDefaults(charDefaults, AllQuestForeverCharDB)
    return AllQuestForeverCharDB
end
